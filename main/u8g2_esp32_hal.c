...
uint8_t u8g2_esp32_msg_comms_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr) {
	...
	switch(msg) {
		...
		case U8X8_MSG_BYTE_SEND: {
			...
			trans_desc.address_bits=0;
			//trans_desc.addr      = 0;
			trans_desc.command_bits=0;
			//trans_desc.command   = 0;
			trans_desc.base.flags     = 0;
			trans_desc.base.length    = 8 * arg_int; // Number of bits NOT number of bytes.
			trans_desc.base.rxlength  = 0;
			trans_desc.base.tx_buffer = arg_ptr;
			trans_desc.base.rx_buffer = NULL;

			//ESP_LOGI(tag, "... Transmitting %d bytes.", arg_int);
			ESP_ERROR_CHECK(spi_device_transmit(handle, &trans_desc.base));
			break;
		}
	}
	return 0;
